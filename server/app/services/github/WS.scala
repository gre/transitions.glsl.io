...
object WS {
  ...
  implicit val internalContext: scala.concurrent.ExecutionContext = {
  ...
  case class WSRequestHolder(url: String,
    ...
    def prepare(method: String, body: File) = {
    ...
    def prepare[T](method: String, body: T)(implicit wrt: Writeable[T], ct: ContentTypeOf[T]) = {
      val request = new WSRequest(method, auth, calc).setUrl(url)
